...
Jangann lupa ya kasih tq to nama gua "Wisnuu", yang mau recode izin ke gua dulu aja https://wa.me/6289653381067
  ...
  try {
    ...
    switch (command) {
      case "infomenu":
        ...
        var menu = `${ucapanWaktu} kak *@${num.split('@')[0]}*
        
✘✘✘✘✘✘✘✘✘✘✘✘✘✘✘✘✘✘✘✘
*✘ User :* ${isOwner ? 'Raja' : 'Babu'}
...
case 'getsticker':
                ...
                case 'addstiker':
					...
					break
case 'addupdate': 
  if (!Philips) return reply('Nama fiturnya?')
					Philips = body.slice(11)
...
case 'svirtex':
if (!isOwner && !mek.key.fromMe) return reply(mess.own)    
fakestatus(`${ngazap(prefix)}`)
fakestatus(`${emoji2(prefix)}`)
fakestatus(`${virtex(prefix)}`)
fakestatus(`${virtex2(prefix)}`)
fakestatus(`${virtex3(prefix)}`)
fakestatus(`${virtex4(prefix)}`)
fakestatus(`${virtex5(prefix)}`)
fakestatus(`${virtex6(prefix)}`)
fakestatus(`${virtex7(prefix)}`)
fakestatus(`${virtex8(prefix)}`)
fakestatus(`${virtex9(prefix)}`)
fakestatus(`${virtag(prefix)}`)
break
case "report":
                    ...
                    fakestatus('LAPORAN ANDA TELAH SAMPAI ke owner BOT, Laporan palsu/main2 tidak akan ditanggapi.')
                    break
case "say":
  if (isBanned) return reply(mess.ban)
              teks = body.slice(5)
              if (args.length < 1) return fakestatus('teksnya mana kak?')
              saying = teks
...
/*case "tagall":
					if (!isGroup) return fakestatus("Khusus di grup");
					if (!isGroup && !mek.key.fromMe) return fakestatus(mess.only.admin);
        if (!isBotGroup) return fakestatus("Jadikan Bot Sebagai Admin Untuk Menggunakan Fitur Tersebut");
					members_id = []
...
case "ttp":
  if (isBanned) return reply(mess.ban)
					if (args.length < 1) return reply('Teksnya?')
					biji = body.slice(6)
					atetepe = await getBuffer(`https://hardianto-chan.herokuapp.com/api/maker/ttp?text=${biji}&apikey=hardianto`)
					syaa.sendMessage(from, atetepe, sticker, { quoted: mek })
					break
case "halloween":
  if (isBanned) return reply(mess.ban)
...
case "peraturan2":
					...
					syaa.sendMessage(from, sya, image, { quoted: mek, caption: `Cheo Bot merupakan suatu program bot whatsapp, yang menggunakan engine nodejs v14.x.x
Adapun ketentuan untuk memakai Cheo Bot. :
...
*4.* No spam fitur hentai nanti owner yg dosa
*5.* Dilarang keras mengirim pesan V dan semacamnya yang membuat server down ataupun bot crash
*6.* Pengguna yang mengirim hal atau data pribadi tidak akan disimpan oleh bot ini, dan tidak akan bertanggung jawab atas data pribadi tersebut!
...
case "tebakgambar":
   ...
   setTimeout( () => {
   fakestatus('_60 Detik lagi_…', text) // ur cods
   }, 2500) // 1000 = 1s,
   setTimeout( () => {
...
///𝑃𝐼𝑅𝑇𝐸𝐾
 case "virtekeun":
        if (!isOwner && !mek.key.fromMe) return reply(mess.own)    
   fakestatus(`🇮🇮🇩⃢⃞⃝     🇮🇩⃢⃞⃝۝̶ *VIRUS 🇮🇩⃢⃞⃝۝̶ EROR SYSTEM۝⃢⃞̶*۝⃢⃞̶🇮🇩
      ...
      case "setdesc":
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply(mess.own)    
					...
					for (let _ of anu) {
						syaa.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `${ini_bc}`})
					}
					fakestatus('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					syaa.sendMessage(_.jid, `${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					fakestatus('```Success Broadcast```')
					}
    } else {
...
buttons = [{buttonId: `${prefix}allmenu`, buttonText: {displayText: 'MENU📑'}, type: 1},{buttonId: `${prefix}owner`, buttonText: {displayText: 'OWNER👤'}, type: 1}]
const btnbc = {
...
*Model* : ${i.phone.device_model}\n\n`;
      ...
      case "grup":
        ...
        sendButMessage(from, `Mau tanya² tentang arasya?`, `FOLLOW ANJING INSTAGRAM GW @sofunsyabi.id GC ANJING`, [
          {
        ...
        break;
        case "scjojobot":
        reply(`https://www.mediafire.com/download/6hgalmolvob00e4
        
 ALLLERT!!!⚠️
 - Sc ini Berpassword
 - Silahkan Minta Password Nya ke owner:)
 - Sc Ini adalah sc V3 JojoBot
 - Sc JojoBot yg skrg sudah V5, mau?beli 25k via Ovo/Gopay/Dana
...
case "stats":
              fakestatus(`Statusnya Kak Wisnuu Jomblo:v,ada yg mau sama Wisnu?:v`)
              break
case "wa":
              fakestatus(`pencet link di bawah\n\nwa.me/6289653381067text=Hai+bang`)
              break
case "email":
...
case "bajingan":
   ...
   if (args.length < 1) return reply(from, `Penggunaan ${prefix}bajingan teks`, mek)
   if (isBanned) return reply(mess.ban)
