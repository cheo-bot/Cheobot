...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Cheo Bot", "Dekstop", "3.0"];
  ...
  fs.existsSync("./Cheo.json") && client.loadAuthInfo("./Cheo.json");
  client.on("connecting", () => {
    start("2", "Menyambung.....");
  });
  client.on("open", () => {
    success("2", "Tersambung√, Hai owner Cheo👋");
  });
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(client.base64EncodedAuthInfo(), null, "\t")
        ...
        ownerNumber = ["6289653381067@s.whatsapp.net",`6289653381067@s.whatsapp.net`]
        dtod = "6289653381067@s.whatsapp.net"
       otod = `6289653381067@s.whatsapp.net`
  ...
  client.on('group-participants-update', async (anu) => {
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Si @${num.split('@')[0]} mental nya down mkny keluar\nahahah canda\nbyee`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
